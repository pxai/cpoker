...
{
	int i = 0;
	char * result[2] = {""," *WINNER* "};
	msg("Showing player hands: \n");
	
	for (i=0;i<PLAYERS;i++)
		msg("\t\tPlayer %d Hand: %s %s %d %d$ - %s:(%d) : %s\n",i,players[i].playerhand[0].show,players[i].playerhand[1].show,players[i].choice,players[i].money,players[i].handval.name,players[i].handval.value,result[players[i].wins]);
}
